class Principal {
  ...
  //metodo para actualizar el movimiento de los enemigos dependiendo el que se este mostrando
  actualizarEnemigos() {
    
   //validamos si nos encontramos en la pantalla 4 y si jugar es true si no estamos en esta pantalla o juaga r es false no 
   //se va ejecutar nada, pero cuando estemos se empieza a ejecutar el juego
  ...
  for (let i = 0; i < this.enemigos.length; i++) {//mediante el ciclo for recorremos el arreglo donde esta al cantidad de enemigos que se van a mostrar
   ...
   //verificamos si los enemigos chacoron con el jugador
 ...
 // Método para reiniciar el juego, restauramos todos los valores
  reiniciarJuego() {
    this.jugar = true;//el jugador puede juagar nuevamente
    this.puntaje = 0;
    ...
    // al precionar el boton detenemos los sonidos en caso de que se esten reproduciendo
  ...
  mostrar() {
    ...
    rect(20,155,600,180);
    fill('tomato');
    text('Evita a las vigilantes y logra escapar del manicomio utiliza las teclas "A" y "D" para moverte de derecha a izquierda entre los carriles',30,170,600);
  }
 }
